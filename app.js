...
// ─── GIF SEARCH TERMS ─────────────────────────────────────────────────────────
const GIF_TERMS = {
  e001:'barbell bench press', e002:'incline dumbbell press', e003:'decline bench press',
  e004:'pec deck fly', e005:'chest dip', e006:'cable crossover',
  e007:'dumbbell pullover', e008:'push up', e009:'cable chest fly',
  e010:'pull up', e011:'barbell bent over row', e012:'lat pulldown',
  e013:'seated cable row', e014:'barbell deadlift', e015:'dumbbell one arm row',
  e016:'straight arm pulldown', e017:'barbell shrug', e018:'hyperextension',
  e020:'barbell squat', e021:'leg press', e022:'romanian deadlift',
  e023:'leg extension', e024:'lying leg curl', e025:'standing calf raise',
  e026:'dumbbell lunge', e027:'bulgarian split squat', e028:'hack squat',
  e029:'sumo deadlift',
  e030:'barbell overhead press', e031:'dumbbell lateral raise',
  e032:'dumbbell front raise', e033:'cable face pull', e034:'arnold press',
  e035:'upright row', e036:'dumbbell shoulder press',
  e040:'dumbbell biceps curl', e041:'triceps dip', e042:'cable triceps pushdown',
  e043:'barbell curl', e044:'hammer curl', e045:'lying triceps extension',
  e046:'concentration curl', e047:'preacher curl', e048:'skull crusher',
  e050:'plank', e051:'crunch', e052:'ab wheel rollout',
  e053:'hanging leg raise', e054:'cable crunch', e055:'russian twist',
  e056:'bicycle crunch',
  e060:'barbell hip thrust', e061:'cable glute kickback', e062:'glute bridge',
  e070:'treadmill running', e071:'stationary bike', e072:'rowing machine',
  e073:'jump rope', e074:'burpee',
};
...
const ExGif = {
  ...
  APIS: [
    // ExerciseDB OSS free tier
    id => `https://oss.exercisedb.dev/api/v1/exercises?search=${encodeURIComponent(GIF_TERMS[id]||id)}&limit=3`,
    // wger public API (exercise images, different structure)
    id => `https://wger.de/api/v2/exercise/search/?term=${encodeURIComponent(GIF_TERMS[id]||id)}&language=es&format=json`,
  ],
  ...
  // Clear broken entries (URLs that loaded with error icon)
  clearBroken() {
    const cache = this._load();
    let changed = false;
    for (const [id, entry] of Object.entries(cache)) {
      if (entry.broken) { delete cache[id]; changed = true; }
    }
    if (changed) this._save();
  },

  // Mark a URL as broken so we retry next time
  markBroken(id) {
    const cache = this._load();
    if (cache[id]) { cache[id].broken = true; delete cache[id].gifUrl; this._save(); }
  },
  ...
  isMissing(id) {
    const e = this._load()[id];
    return e && !e.gifUrl && !e.broken && e.notFound;
  },

  // Validate that an image URL actually loads before caching it
  _testImg(url) {
    return new Promise(resolve => {
      ...
      img.src = url;
      setTimeout(() => resolve(null), 6000); // 6s timeout
    });
  },
    ...
    const p = (async () => {
      try {
        ...
        const res = await fetch(
          `https://oss.exercisedb.dev/api/v1/exercises?search=${encodeURIComponent(q)}&limit=3`,
        ...
        if (res.ok) {
          ...
          // Try each result until one image actually loads
          for (const item of list) {
            const url = item?.gifUrl || item?.image || null;
            if (!url) continue;
            const valid = await this._testImg(url);
            if (valid) {
              cache[id] = { gifUrl: valid, t: Date.now() };
              this._save();
              return valid;
            }
          }
        }
      } catch(e) { /* network error or timeout — silent */ }

      // Mark as not found (don't retry every time)
    ...
    ${exs.map(e=>{
      ...
      return `<div class="expage-item" onclick="App.openExDetail('${e.id}')">
        <div class="expage-thumb" id="expthumb-${e.id}"
          ...
          ${cached?`<img src="${cached}" loading="lazy" style="width:100%;height:100%;object-fit:cover">`:''}
          ${!cached?thumbSvg():''}
        </div>
...
const App = {
  init() {
    ...
    // ── Clear broken GIF cache entries so they get retried ────────────────
    ExGif.clearBroken();
