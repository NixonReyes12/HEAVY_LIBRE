...
// ─── EXERCISE DATABASE ────────────────────────────────────────────────────────
// Secondary muscles per exercise id (mirrors Hevy's "Secundario" field)
const SECONDARY_MUSCLES = {
  e001:['Hombros','Brazos'], e002:['Hombros','Brazos'], e003:['Hombros','Brazos'],
  e004:['Hombros','Brazos'], e005:['Hombros','Brazos'], e006:['Hombros'],
  e010:['Brazos','Hombros'], e011:['Brazos','Hombros'], e012:['Brazos'],
  e013:['Brazos'], e014:['Piernas','Core','Glúteos'], e015:['Brazos'], e016:['Brazos','Core'],
  e020:['Glúteos','Core'], e021:['Glúteos'], e022:['Glúteos','Espalda'],
  e023:[], e024:['Glúteos'], e025:[], e026:['Glúteos','Core'], e027:['Glúteos','Core'],
  e030:['Brazos','Core'], e031:[], e032:[], e033:['Espalda'], e034:['Brazos'],
  e040:[], e041:['Pecho','Hombros'], e042:[], e043:[], e044:[], e045:[], e046:[],
  e050:['Hombros'], e051:[], e052:['Hombros'], e053:[], e054:[],
  e060:['Piernas','Core'], e061:['Core'],
...
function _exSessionsInRange(exId) {
  let sessions = S.history
    .filter(w => w.exercises?.some(e => e.id === exId))
    .sort((a,b) => new Date(a.date)-new Date(b.date));
  const days = S.exRangeDays ?? 90;
  if (days > 0) {
    const cut = Date.now() - days*86400000;
...
function renderExGraph(exId, mode) {
  ...
  if (!canvas) return;
  const sessions = _exSessionsInRange(exId);
  const W = canvas.offsetWidth || 340;
  ...
  // Highlight: most recent value + date
  const hlValEl  = document.getElementById('exd-highlight-val');
  const hlDateEl = document.getElementById('exd-highlight-date');
  if (sessions.length) {
    const last = sessions[sessions.length-1];
    const v = valueFor(last);
    const unitLbl = mode === 'volume' ? ` ${S.unit}` : (mode === '1rm' ? ` ${S.unit} (est.)` : ` ${S.unit}`);
    hlValEl.textContent = `${v}${unitLbl}`;
    hlDateEl.textContent = fmtDateShort(last.date);
  } else {
    hlValEl.textContent = '—';
    hlDateEl.textContent = '';
  }

  if (sessions.length < 2) {
    ctx.fillStyle='#48484f'; ctx.font='12px sans-serif'; ctx.textAlign='center';
    ctx.fillText('Necesitas al menos 2 sesiones en este rango', W/2, 84);
    return;
  }

  const vals = sessions.map(valueFor);
  const lo = Math.min(...vals)*0.97, hi = Math.max(...vals)*1.03 || 1;
  const X = i => 22 + i*(W-44)/(sessions.length-1);
  const Y = v => 130 - ((v-lo)/(hi-lo || 1))*105;

  // Gridlines + axis labels (top/mid/bottom like Hevy)
  ctx.strokeStyle = 'rgba(255,255,255,.06)'; ctx.lineWidth = 1;
  [hi, (hi+lo)/2, lo].forEach((v, idx) => {
    const y = Y(v);
    ctx.beginPath(); ctx.moveTo(22, y); ctx.lineTo(W-22, y); ctx.stroke();
    ctx.fillStyle = '#48484f'; ctx.font = '10px sans-serif'; ctx.textAlign = 'left';
    ctx.fillText(`${Math.round(v)}`, 2, y - (idx===2 ? -10 : 3));
  });
  ...
  ctx.beginPath(); ctx.moveTo(X(0),Y(vals[0]));
  for (let i=1;i<vals.length;i++) ctx.lineTo(X(i),Y(vals[i]));
  ctx.strokeStyle='#5b8dee'; ctx.lineWidth=2.5; ctx.lineJoin='round'; ctx.stroke();
  vals.forEach((v,i) => {
    ctx.beginPath(); ctx.arc(X(i),Y(v),4,0,Math.PI*2);
    ctx.fillStyle='#5b8dee'; ctx.fill();
  });

  // Date labels (first / last only, like Hevy when range has few points)
  ctx.fillStyle='#48484f'; ctx.font='9px sans-serif'; ctx.textAlign='center';
  ctx.fillText(fmtDateShort(sessions[0].date), X(0), 150);
  ctx.fillText(fmtDateShort(sessions[sessions.length-1].date), X(sessions.length-1), 150);
}

function renderExPRCard(exId) {
  let bestWeight = 0, bestRM = 0, bestVolSet = 0, bestVolLabel = '';
  for (const w of S.history) {
    const ex = w.exercises?.find(e => e.id === exId);
    if (!ex) continue;
    for (const s of (ex.sets||[])) {
      ...
      if (rm && rm > bestRM) bestRM = rm;
      const vol = wt*rp;
      if (vol > bestVolSet) { bestVolSet = vol; bestVolLabel = `${wt}${S.unit} x ${rp}`; }
    }
  }
  document.getElementById('exd-pr-weight').textContent = bestWeight ? `${bestWeight}${S.unit}` : '—';
  document.getElementById('exd-pr-1rm').textContent     = bestRM ? `${bestRM}${S.unit}` : '—';
  document.getElementById('exd-pr-vol').textContent     = bestVolLabel || '—';
}
...
const App = {
  ...
  // ── Exercise detail ───────────────────────────────────────────────────────
  openExDetail(exId) {
    S.exGraphMode = 'best_weight';
    S.exRangeDays = 90;
    document.querySelectorAll('#modal-ex-detail .gtab').forEach((t,i) => t.classList.toggle('active', i===0));
    renderExDetail(exId);
    this.open('modal-ex-detail');
  },
  switchExGraph(mode, btn) {
    S.exGraphMode = mode;
    document.querySelectorAll('#modal-ex-detail .gtab').forEach(t => t.classList.remove('active'));
    btn.classList.add('active');
    renderExGraph(S.currentExId, mode);
  },
  changeExRange(days) {
    S.exRangeDays = parseInt(days);
    renderExGraph(S.currentExId, S.exGraphMode);
  },
