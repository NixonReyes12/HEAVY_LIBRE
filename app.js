...
const ExGif = {
  ...
  getCached(id) { const e = this._load()[id]; return e?.gifUrl || null; },
  isMissing(id) { const e = this._load()[id]; return e && e.gifUrl === null; },
  async fetch(id) {
    const info = allEx().find(e => e.id === id);
    if (!info || info.custom) return null;
    const cache = this._load();
    if (cache[id] !== undefined) return cache[id].gifUrl;
    if (this._pending[id]) return this._pending[id];
    const q = GIF_TERMS[id] || info.name;
    const p = (async () => {
      try {
        const res = await fetch(`https://oss.exercisedb.dev/api/v1/exercises?search=${encodeURIComponent(q)}&limit=1`);
        if (!res.ok) throw new Error();
        const data = await res.json();
        const list = data?.data || data?.exercises || data || [];
        const gifUrl = Array.isArray(list) && list[0]?.gifUrl ? list[0].gifUrl : null;
        cache[id] = {gifUrl, t: Date.now()};
        this._save();
        return gifUrl;
      } catch(e) { return null; }
      finally { delete this._pending[id]; }
    })();
    this._pending[id] = p;
    return p;
  },
...
function rExDetail(exId) {
  ...
  if (!info?.custom) {
    ...
    if (cached) showGif(cached);
    else ExGif.fetch(exId).then(url=>{
      if (S.currentExId===exId) url?showGif(url):(gifStatus.textContent='No disponible');
    });
  }
...
function showGif(url) {
  ...
  img.onerror=()=>{ const s=document.getElementById('exd-gif-status'); if(s)s.textContent='No disponible'; };
  img.src=url;
}
