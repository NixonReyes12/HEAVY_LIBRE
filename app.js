...
function muscleThumb(muscle, size='md'){
  const color  = MUSCLE_COLORS[muscle] || '#7c6af7';
...
function rActivityDots(){
  ...
  for(let i=6;i>=0;i--){
    ...
    const ws=S.history.filter(w=>localDateStr(w.date)===ds);
...
function rHeatmap(){
  ...
  for(let i=181;i>=0;i--){
    ...
    const ds=localDateStr(d);
    const count=S.history.filter(w=>localDateStr(w.date)===ds).length;
...
function rMiniCal(){
  ...
  for(let d=1;d<=days;d++){
    ...
    const hasW=S.history.some(w=>localDateStr(w.date)===ds);
...
function rVolChart(){
  ...
  ctx.fillStyle='#44447a'; ctx.font='9px -apple-system,sans-serif'; ctx.textAlign='center';
...
function rWeightChart(){
  ...
  if(pts.length<2){
    ctx.fillStyle='#44447a';ctx.font='12px sans-serif';ctx.textAlign='center';
  ...
  grad.addColorStop(0,'rgba(124,106,247,.22)');grad.addColorStop(1,'rgba(124,106,247,0)');
...
function rHistory(){
  ...
  el.innerHTML=list.map(w=>{
    ...
    const rankChip=S.mode==='competitive'
      ?`<span class="rank-chip-sm" style="background:${hexAlpha('#7c6af7',.12)};color:#a78bfa;border:1px solid ${hexAlpha('#7c6af7',.3)}">${current.icon} ${current.label}</span>`
      :'';
  ...
  return`<div class="ex-block ${isSuper?'is-superset':''}" id="exb-${bi}">
    ...
    <table class="sets-tbl">
      <thead><tr>
        ...
        <th>REPS</th><th>RPE</th><th>ANT.</th><th>✓</th><th></th>
      </tr></thead>
...
function rSetRow(s,bi,si,exType){
  ...
  return`<tr id="sr-${bi}-${si}" class="${s.done?'set-done':''}">
    ...
    <td><button class="del-set-btn" onclick="App.delSet(${bi},${si})">−</button></td>
  </tr>`;
}
...
function rRFExList(){
  ...
  if(!S.rfExercises.length){
    el.innerHTML=`<p class="empty-state" style="padding:10px 0">Sin ejercicios — añade con el botón de arriba.</p>`;
    return;
  }
  el.innerHTML=S.rfExercises.map((ex,bi)=>{
    const info=findEx(ex.id);
    const exType=info?.type||'weight_reps';
    const isDur=exType==='duration', isReps=exType==='reps_only', isBW=exType==='bodyweight_reps';
    const weightHeader=isDur?'SEG':isReps?'—':'PESO';
      ...
      const weightCell=isBW
        ...
        :isReps
          ...
          :`<input class="w-inp" type="number" step="${isDur?1:.5}" value="${s.w||''}" placeholder="0"
              oninput="App.updRFSet(${bi},${si},'w',this.value)" style="width:48px"/>`;
      return`<tr>
        <td style="padding:5px 3px;text-align:center">
          <button class="stype-btn ${s.type||'N'}" onclick="App.cycleRFType(${bi},${si})">${s.type||'N'}</button>
        </td>
        <td style="padding:5px 3px;text-align:center">${weightCell}</td>
        <td style="padding:5px 3px;text-align:center">
          <input class="r-inp" type="number" value="${s.r||''}" placeholder="0"
            oninput="App.updRFSet(${bi},${si},'r',this.value)" style="width:48px"/>
        </td>
        <td style="padding:5px 3px;text-align:center">
          <button class="del-set-btn" onclick="App.delRFSet(${bi},${si})">−</button>
        </td>
      </tr>`;
    }).join('');
    return`<div class="rfe-block" id="rfeb-${bi}">
      <div class="rfe-head">
        <div>
          <div class="rfe-name">${info?.name||'?'}</div>
          <div class="rfe-sub">${info?.muscle||''} · ${info?.equipment||''}</div>
        </div>
        <div style="display:flex;align-items:center;gap:6px">
          <span style="font-size:11px;color:var(--t3)">${(ex.sets||[]).length} series</span>
          <button onclick="App.removeRFEx(${bi})" style="color:var(--t3);font-size:20px;padding:2px 6px;line-height:1">×</button>
        </div>
      </div>
      <table class="sets-tbl">
        <thead><tr>
          <th>TIPO</th><th>${weightHeader}</th><th>REPS</th><th></th>
        </tr></thead>
        <tbody id="rftb-${bi}">${rows}</tbody>
      </table>
      <div style="padding:8px 14px;border-top:1px solid var(--border)">
        <button class="btn-add-set" onclick="App.addRFSet(${bi})">+ Añadir serie</button>
      </div>
    </div>`;
  }).join('');
}

function refreshRFSets(bi){
  const ex=S.rfExercises[bi];
  const info=findEx(ex.id);
  const exType=info?.type||'weight_reps';
  const isDur=exType==='duration', isReps=exType==='reps_only', isBW=exType==='bodyweight_reps';
  const tbody=document.getElementById(`rftb-${bi}`);
  if(!tbody) return;
  tbody.innerHTML=(ex.sets||[]).map((s,si)=>{
    const weightCell=isBW
      ?'<span style="font-size:10px;color:var(--t3)">BW</span>'
      :isReps
        ?'<span style="font-size:10px;color:var(--t3)">—</span>'
  ...
  // Update series count label
  const head=document.querySelector(`#rfeb-${bi} .rfe-head span`);
  if(head) head.textContent=`${(ex.sets||[]).length} series`;
}
...
function rExGraph(exId,mode){
  ...
  if(sessions.length<2){ctx.fillStyle='#44447a';ctx.font='12px sans-serif';ctx.textAlign='center';ctx.fillText('Necesitas al menos 2 sesiones en este rango',W/2,80);return;}
  ...
  [hi,(hi+lo)/2,lo].forEach(v=>{const y=Y(v);ctx.beginPath();ctx.moveTo(22,y);ctx.lineTo(W-22,y);ctx.stroke();ctx.fillStyle='#44447a';ctx.font='9px sans-serif';ctx.textAlign='left';ctx.fillText(Math.round(v),2,y-2);});
  ...
  vals.forEach((v,i)=>{ctx.beginPath();ctx.arc(X(i),Y(v),4,0,Math.PI*2);ctx.fillStyle='#a78bfa';ctx.fill();});
  ctx.fillStyle='#44447a';ctx.font='9px sans-serif';ctx.textAlign='center';
...
const App = {
  ...
  // Routine form set operations
  addRFSet(bi){
    const last=S.rfExercises[bi].sets.at(-1);
    S.rfExercises[bi].sets.push({type:last?.type||'N',w:last?.w||'',r:last?.r||'',rpe:'',done:false,prev:null});
    refreshRFSets(bi);
  },
  delRFSet(bi,si){
    if(S.rfExercises[bi].sets.length<=1){toast('Mínimo 1 serie por ejercicio.');return;}
    S.rfExercises[bi].sets.splice(si,1);
    refreshRFSets(bi);
  },
  updRFSet(bi,si,field,val){
    const s=S.rfExercises[bi].sets[si];
    s[field]=['w','r'].includes(field)?(parseFloat(val)||0):val;
  },
  cycleRFType(bi,si){
    const types=['N','W','D','F','S'];
    const s=S.rfExercises[bi].sets[si];
    s.type=types[(types.indexOf(s.type||'N')+1)%types.length];
    refreshRFSets(bi);
  },
  ...
  selectEx(exId){
    ...
    if(S.pickerCtx==='live'&&S.live){
      ...
      // When adding to routine form, start with 3 clean sets with type N
      newEx.sets=[
        {type:'N',w:'',r:'',rpe:'',done:false,prev:null},
        {type:'N',w:'',r:'',rpe:'',done:false,prev:null},
        {type:'N',w:'',r:'',rpe:'',done:false,prev:null},
      ];
  ...
  delSet(bi,si){
    ...
    if(exs[bi].sets.length<=1){toast('Mínimo 1 serie por ejercicio.');return;}
    exs[bi].sets.splice(si,1);
    refreshSets(bi);
    if(S.live) updVolChip();
  },
  ...
  startFromRoutine(id){
    ...
    const exercises=(r.exercises||[]).map(ex=>{
      ...
      // Source of truth for NUMBER of sets = the routine definition
      // History is only used to fill in the "prev" hint (what you did last time)
      const routineSets=ex.sets?.length?ex.sets:[{type:'N'}];

      // Get last session history for this exercise (for prev hints)
      const lastW=[...S.history].reverse().find(w=>w.exercises?.some(e=>e.id===ex.id));
      const lastEx=lastW?.exercises?.find(e=>e.id===ex.id);
      // All sets from last session (not just completed ones) — use by index
      const lastSets=(lastEx?.sets||[]);

      // Build sets matching routine count, with prev hints from history by position
      newEx.sets=routineSets.map((s,i)=>{
        // Find best matching prev set: same index, must have been done
        const histSet=lastSets.filter(ls=>ls.done&&ls.type!=='W')[i]||null;
        return {
          type:s.type||'N',
          w:'',r:'',rpe:'',done:false,
          prev:histSet?{w:histSet.w,r:histSet.r}:null,
        };
      });
  ...
  finishWorkout(){
    ...
    S.history.push({id:uid(),name:S.live.name,date:today(),duration:fmtTime(elapsed),exercises:exs,volume:vol,sessionNote:S.live.sessionNote||''});
